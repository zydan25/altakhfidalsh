(() => {
  ...
  // - On first load, only the section containing the current route is open.
  // - Opening one section closes all other sections.
  // - No localStorage is used, so old saved states can never reopen every section.
  const setSectionOpen = (target, open) => {
    sections.forEach((section) => {
      const button = section.querySelector(".nav-section-button");
      const children = section.querySelector(".nav-children");
      const isOpen = section === target ? open : false;
  ...
  const activeSection = sections.find((section) =>
    section.classList.contains("is-active-parent")
  );
  ...
  setSectionOpen(activeSection || null, Boolean(activeSection));
