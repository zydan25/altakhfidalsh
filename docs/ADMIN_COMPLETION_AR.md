...
## الفئات الجانبية ومؤقت الترند

### الفئات الجانبية

تمت إضافة منظومة مستقلة:

`Category Root → SideCategory → SideCategoryCircle → ProductSideCategoryCircle`

القسم الجانبي لا يرتبط إلا بـCategory من المستوى الأعلى (`parent_id = NULL`). دوائر التصنيف الجانبية مستقلة عن `categories` ويمكن رفع صورة لها، ترتيبها، أرشفتها وربط المنتجات بها.

في معالج المنتج توجد خطوة مستقلة لاختيار دوائر الفئات الجانبية التي يظهر المنتج تحتها، مع البحث وحالة التحديد وعدد المنتجات.

### مؤقت الترند

الترند المستطيل يدعم الآن:

- مؤقت اختياري.
- وحدة المؤقت ثوانٍ أو دقائق.
- وقت بدء محسوب عند حفظ الترند النشط.
- `ends_at` في الاستجابة العامة.
- انتهاء المؤقت يمنع الترند من الظهور في الـAPI العام.
- نص Overlay اختياري.
- لون الخط ولون خلفية النص.
